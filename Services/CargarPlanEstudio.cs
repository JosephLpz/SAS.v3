            ...
            if (utlXls.init(path, NombreHoja))
            {
                ...
                while (continuar)
                {
                   ...
                   // RequisitosAsignatura reqAsignaturas = new RequisitosAsignatura();
                    ...
                    string semestre= utlXls.getCellValue(string.Format("A{0}", fila));
                    if (number.IsNumeric(semestre))
                    {
                       
                       // string codigoAsignatura= utlXls.getCellValue(string.Format("D{0}", fila));

                        string asignatura = utlXls.getCellValue(string.Format("B{0}", fila));

                        

                        string catedra = utlXls.getCellValue(string.Format("D{0}", fila));
                        if (catedra == "")
                        {
                            catedra = "0";
                        }

                        string taller = utlXls.getCellValue(string.Format("E{0}", fila));

                        if (taller == "")
                        {
                            taller = "0";
                        }
                        string laboratorio= utlXls.getCellValue(string.Format("F{0}", fila));
                        ...
                        string PracticaCurricular= utlXls.getCellValue(string.Format("G{0}", fila));

                        string SCT= utlXls.getCellValue(string.Format("H{0}", fila));

                        string asignaturaRequisito= utlXls.getCellValue(string.Format("I{0}", fila));
